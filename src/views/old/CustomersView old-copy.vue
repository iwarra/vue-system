...
let items = ref([]); // fetch here already
const loadingStore = useLoadingStore(); // just check if reportData.value exists
...
onMounted(async () => {
	items.value = await fetchData("customer", items.value);
...
// needs refactoring to access the dom with refs
